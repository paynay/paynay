...
{
    /* current difficulty formula, paynay - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
...
unsigned int GetLegacyNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock, bool fProofOfStake)
{
    ...
    // PAYNAY coin: target change every block
    // PAYNAY coin: retarget with exponential moving toward target spacing
    uint256 bnNew;
    bnNew.SetCompact(pindexPrev->nBits);
